...
void escrever_registro_cliente(Cliente c, int rrn) {
    char * p = ARQUIVO_CLIENTES + TAM_REGISTRO_CLIENTE + rrn;
...
void btree_insert(char *chave, btree *t) {
    ...
    aux_node.qtd_chaves = 0;

    if (t->rrn_raiz == -1){
        strcpy(aux_node.chaves[0],chave);
        aux_node.qtd_chaves++;
        aux_node.folha = true;
...
promovido_aux btree_insert_aux(char *chave, int rrn, btree *t) {
    ...
    aux_node = btree_read(rrn, t);

    if (aux_node.folha){
        if (aux_node.qtd_chaves < btree_order - 1){

            for (i = aux_node.qtd_chaves; i > 0 &&  0 < t->compar((void*)chave, (void*)aux_node.chaves[i]); i--)
                strcpy( aux_node.chaves[i], aux_node.chaves[i-1]);
            
            strcpy(aux_node.chaves[i],chave);
            aux_node.chaves++;
        ...
        promo_aux_node = btree_insert_aux(chave, aux_node.filhos[i], t);

        if (promo_aux_node.filho_direito != -1){
            if( aux_node.qtd_chaves < btree_order){
                for (i = aux_node.qtd_chaves; i > 0 &&  0 < t->compar((void*)promo_aux_node.chave_promovida, (void*)aux_node.chaves[i]); i--){
                    strcpy( aux_node.chaves[i], aux_node.chaves[i-1]);
                    aux_node.filhos[i+1] = aux_node.filhos[i];
                }
                aux_node.filhos[i] = promo_aux_node.filho_direito;
                strcpy(aux_node.chaves[i],promo_aux_node.chave_promovida);
...
promovido_aux btree_divide(char *chave, int filho_direito, int rrn, btree *t) {
    int i = btree_order-1;
    bool alocated_key = false;
    promovido_aux promoted;

    btree_node aux_node = btree_node_malloc(t), new_node = btree_node_malloc(t);

    aux_node = btree_read(rrn, t);

    new_node.folha = aux_node.folha;
    new_node.qtd_chaves = (btree_order-1)/2;

    for (int j = (btree_order-1)/2; j > 0; j--){
        if (!alocated_key && 0 < t->compar((void*)chave, (void*)aux_node.chaves[i])){
            strcpy(new_node.chaves[j-1],chave);
            new_node.filhos[j] = filho_direito;
            alocated_key = true;
        }else{
            strcpy(new_node.chaves[j-1], aux_node.chaves[i-1]);
            new_node.filhos[j] = aux_node.filhos[i];
            i--;
        }
    }
    if (!alocated_key){
        for (; i > 0 &&  0 < t->compar((void*)chave, (void*)aux_node.chaves[i]); i--){
                    strcpy( aux_node.chaves[i], aux_node.chaves[i-1]);
                    aux_node.filhos[i+1] = aux_node.filhos[i];
                }
        strcpy(aux_node.chaves[i], chave);
        aux_node.filhos[i+i] = filho_direito;
    }

    i = btree_order/2;

    new_node.filhos[0] = aux_node.filhos[i+1];
    aux_node.qtd_chaves = i;
...
btree_node btree_read(int rrn, btree *t) {
    ...
    p += (t->tam_chave*aux_node.qtd_chaves) - aux_node.qtd_chaves;
...
void btree_write(btree_node no, btree *t) {
    ...
    k = (t->tam_chave * t->qtd_nos) + 1; //mais o byte da folha


    for (i=0; i < no.qtd_chaves; i++){
        
        strpadright(no.chaves[i], '#', t->tam_chave);
